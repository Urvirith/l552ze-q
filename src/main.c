...
/* GPIO Port B REGISTERS */
#define GPIOC_BASE          (uint32_t)0x42020800                            /* GPIO Port B base address */
#define GPIOC_MODER         *((volatile uint32_t *) (GPIOC_BASE  + 0x00))   /* Port B Mode register */
#define GPIOC_OTYPER        *((volatile uint32_t *) (GPIOC_BASE  + 0x04))   /* Port B Output Type Register */
...
extern void _start() { 
    GPIOC_MODER &= (~(MASK_2_BIT << (LED_GRN * 2)));
